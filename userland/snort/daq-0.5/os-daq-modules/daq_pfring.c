  ...
  while ((!context->breakloop) && ((cnt == -1) || (cnt > 0)))
    {
      ...
      ret = pfring_read((current_ring = context->ring_handle), context->pkt_buffer, context->snaplen, &phdr, 0 /* Dont't wait */);

      if((ret == -1) && (context->twin_ring_handle != NULL))
	ret = pfring_read((current_ring = context->twin_ring_handle), context->pkt_buffer, context->snaplen, &phdr, 0 /* Dont't wait */);      
