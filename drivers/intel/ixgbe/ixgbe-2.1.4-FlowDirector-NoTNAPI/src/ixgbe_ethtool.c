...
o		 * this function does not support any duplex forcing, but can
                            ...
                            struct ethtool_eeprom *eeprom, u8 *bytes)
{
...
#ifdef HAVE_PF_RING
	{
	  ...
	  if((bytes != NULL)
		...
		printk("--> ixgbe_set_rx_ntuple [error %d]\n", 2);
	      }
	      return -EINVAL;
	    }
	    ...
	    if(element->rule.rule_type == perfect_filter_rule) {
	      ...
	      if(debug)
		printk("--> ixgbe_set_rx_ntuple(rule_id=%d, add_filter=%d, proto=%d, vlan=%d, sport=%d, dport=%d)\n",
		       element->rule.rule_id, element->add_rule,
		       element->rule.rule.perfect_rule.proto, element->rule.rule.perfect_rule.vlan_id,
		       element->rule.rule.perfect_rule.s_port, element->rule.rule.perfect_rule.d_port);
		...
		ixgbe_atr_set_src_ipv4_82599(&input_struct,
					     (element->rule.rule.perfect_rule.s_addr & ~element->rule.rule.perfect_rule.s_addr));
		ixgbe_atr_set_dst_ipv4_82599(&input_struct,
					     (element->rule.rule.perfect_rule.d_addr & ~element->rule.rule.perfect_rule.d_addr));
		/* 82599 expects these to be byte-swapped for perfect filtering */
		ixgbe_atr_set_src_port_82599(&input_struct,
					     ((ntohs(element->rule.rule.perfect_rule.s_port)) & ~element->rule.rule.perfect_rule.s_port));
		ixgbe_atr_set_dst_port_82599(&input_struct,
					     ((ntohs(element->rule.rule.perfect_rule.d_port)) & ~element->rule.rule.perfect_rule.d_port));

		/* VLAN and Flex bytes are either completely masked or not */
		if (!element->rule.rule.perfect_rule.vlan_id)
		  ixgbe_atr_set_vlan_id_82599(&input_struct, htons(element->rule.rule.perfect_rule.vlan_id));
	      }

	      spin_lock(&adapter->fdir_perfect_lock);
	      if(debug)
		printk("--> ixgbe_fdir_add_perfect_filter_82599(id=%d,target_queue=%d) called\n",
		       element->rule.rule_id, target_queue);
	      ixgbe_fdir_add_perfect_filter_82599(&adapter->hw, &input_struct,
						  &input_masks, element->rule.rule_id, target_queue);
		...
		ixgbe_ftqf_add_filter(&adapter->hw,
	      ...
	      else /* Set the rule to accept all */
		ixgbe_ftqf_add_filter(&adapter->hw,
				      0,
				      0,
				      0,
