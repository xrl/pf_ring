...
u8 bnx2x_direct_parallel_detect_used(struct link_params *params)
{
	struct bnx2x *bp = params->bp;
...
static void bnx2x_an_resolve(struct link_params *params,
				   struct link_vars *vars,
				   u32 gp_status)

{
	...
	DP(NETIF_MSG_LINK, "AN result for port %d: 0x%x\n", params->port,
		 vars->autoneg);
}
				   ...
				   u8 ext_phy_link_up)
{
	...
	if (gp_status & MDIO_GP_STATUS_TOP_AN_STATUS1_LINK_STATUS) {
		...
		if (params->req_line_speed == SPEED_AUTO_NEG)
			bnx2x_an_resolve(params, vars, gp_status);
