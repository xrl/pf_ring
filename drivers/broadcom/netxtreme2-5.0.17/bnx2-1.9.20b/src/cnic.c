...
static int cnic_unregister_device(struct cnic_dev *dev, int ulp_type)
{
	...
	if (rcu_dereference(cp->ulp_ops[ulp_type])) {
		...
		if (dev->use_count == 0) {
			cnic_stop_hw(dev);
#ifdef __VMKLNX__
			if (cp->first_iscsiNetHandle.ptr) {
				vmk_IscsiNetTcpPortFree(
					cp->first_iscsiNetHandle,
			      ...
			      u32 num, int *work)
{
	...
	struct cnic_context *ctx = &cp->ctx_tbl[l5_cid];
#ifdef __VMKLNX__
	char vmknic[VMK_VMKNIC_NAME_MAX];
	...
	/* Retrieve the MAC of the vmknic */
	if (vmk_IscsiNetHandleGetVmknic(cp->iscsiNetHandle, vmknic) != VMK_OK)
		return -EINVAL;
	if (vmk_VmknicMACAddrGet(vmknic, srcMACAddr) != VMK_OK)
		return -EINVAL;

	cnic_init_bnx2x_mac(dev, srcMACAddr);
#else
...
static int cnic_cm_offload_pg(struct cnic_sock *csk)
{
	...
	l4kwqe->sa0 = srcMACAddr[0];
	l4kwqe->sa1 = srcMACAddr[1];
	l4kwqe->sa2 = srcMACAddr[2];
	l4kwqe->sa3 = srcMACAddr[3];
	l4kwqe->sa4 = srcMACAddr[4];
	l4kwqe->sa5 = srcMACAddr[5];
#else
...
static struct dst_entry *cnic_ip6_rte_output(struct sock *sk, struct flowi *fl)
{
	...
	fn = __symbol_get("ip6_route_output");
	if (fn) {
#if (LINUX_VERSION_CODE >= 0x02061a)
...
static int cnic_ipv6_addr_type(const struct in6_addr *addr)
{
	...
	fn = __symbol_get("__ipv6_addr_type");
	if (fn) {
		type = fn(addr) & 0xffff;
		__symbol_put("__ipv6_addr_type");
	}
	return type;
}
			       ...
			       struct in6_addr *saddr)
{
	...
	fn = __symbol_get("ipv6_dev_get_saddr");
	if (fn) {
		rc = fn(&init_net, dst->dev, daddr, 0, saddr);
		__symbol_put("ipv6_dev_get_saddr");
	}
	...
	fn = __symbol_get("ipv6_get_saddr");
	if (fn) {
		rc = fn(dst, daddr, saddr);
		__symbol_put("ipv6_get_saddr");
	}
#endif
	return rc;
}
...
{
	char vmknic[VMK_VMKNIC_NAME_MAX];
	char devName[VMK_UPLINK_DEVICE_NAME_MAX];
	struct cnic_dev *dev;
	int found = 0;
	VMK_ReturnStatus status;

	/* Retrieve the uplink device name corresponding to the vmknic */
	status = vmk_IscsiNetHandleGetVmknic(iscsiNetHandle, vmknic);
	if (status != VMK_OK) 
		return NULL;
	status = vmk_IscsiNetVmknicGetUplinkDevName(iscsiNetHandle, devName);
	if (status != VMK_OK) 
		return NULL;
	...
	list_for_each_entry(dev, &cnic_dev_list, list) {
		...
		if (dev->netdev && test_bit(CNIC_F_IF_UP, &dev->flags)) {
			if (!strcmp(dev->netdev->name, devName)) {
				...
				memcpy(&cp->iscsiNetHandle, &iscsiNetHandle,
				       sizeof(iscsiNetHandle));
				cp->iscsiNetHandle.ptr = cp->vmknic;

				/* Allocate TCP ports, only once per device */
				if (cp->csk_port_tbl.table)
					break;

				strcpy(cp->first_vmknic, vmknic);
				memcpy(&cp->first_iscsiNetHandle,
				       &iscsiNetHandle, sizeof(iscsiNetHandle));
				cp->first_iscsiNetHandle.ptr = cp->first_vmknic;
				cp->cnic_local_port_nr = MAX_CM_SK_TBL_SZ;
				cp->cnic_local_port_min = 1;
				status = vmk_IscsiNetTcpPortAlloc(
						cp->first_iscsiNetHandle,
						&cp->cnic_local_port_nr,
						&cp->cnic_local_port_min);
				if (status != VMK_OK)
...
static int cnic_get_route(struct cnic_sock *csk, struct cnic_sockaddr *saddr)
{
	...
	struct cnic_local *cp = dev->cnic_priv;
	char vmknic[VMK_VMKNIC_NAME_MAX];
	vmk_SocketAddress srcIPAddr, dstIPAddr;
	...
	memcpy(&dstIPAddr, &saddr->remote, sizeof(vmk_SocketIPAddress));
	dstIPAddr.sa_family = VMK_SOCKET_AF_INET;

	/* Next hop (arp) resolve */
	while (retry++ < 10) {
		err = vmk_IscsiNetNextHopMACLookup(cp->iscsiNetHandle, dstIPAddr, nextHopMAC);
		if (nextHopMAC[0] || nextHopMAC[1] || nextHopMAC[2]) {
			if (!err)
				break;
		}
		msleep(100);
	...
	if (vmk_IscsiNetPmtuLookup(cp->iscsiNetHandle, dstIPAddr, &pmtu) == VMK_OK)
		csk->pmtu = pmtu;
	else
		csk->pmtu = 1500;

	/* Retrieve the IP Address of the vmknic */
	srcIPAddr.sa_family = VMK_SOCKET_AF_INET;
	if (vmk_IscsiNetHandleGetVmknic(cp->iscsiNetHandle, vmknic) != VMK_OK)
		return -EINVAL;
	if (vmk_VmknicIPAddrGet(vmknic, &srcIPAddr) != VMK_OK)
		return -EINVAL;

	csk->src_ip[0] = ((vmk_SocketIPAddress *)&srcIPAddr)->sin_addr.s_addr;
	csk->dst_ip[0] = saddr->remote.v4.sin_addr.s_addr;
	csk->src_port = htons(local_port);
	csk->dst_port = saddr->remote.v4.sin_port;

	if (vmk_IscsiNetVmknicGetVlanID(cp->iscsiNetHandle, &vlanId) != VMK_OK)
		return -EINVAL;
...
#ifdef __VMKLNX__
	vmk_IscsiNetDriverData iscsinet_drv_data; 
	struct cnic_eth_dev *(*bnx2_cnic_probe)(struct net_device *);
	VMK_ReturnStatus vm_err;

	vm_err = vmk_IscsiNetDriverRegistered(vmk_ModuleGetID("bnx2"), &iscsinet_drv_data);
	if (vm_err != VMK_OK)
		return NULL;
 
	bnx2_cnic_probe = iscsinet_drv_data.drvData;
...
static struct cnic_dev *init_bnx2x_cnic(struct net_device *dev)
{
	...
	vm_err = vmk_IscsiNetDriverRegistered(vmk_ModuleGetID("bnx2x"), &iscsinet_drv_data);
	if (vm_err != VMK_OK)
		return NULL;
 
	bnx2x_cnic_probe = iscsinet_drv_data.drvData;
