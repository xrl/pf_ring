...
#define DRV_MODULE_VERSION	"1.9.20b"
#define DRV_MODULE_RELDATE	"July 9, 2009"
...
static int disable_netq = 0;

module_param(disable_netq, int, 0);
MODULE_PARM_DESC(disable_netq, "Disable NetQueue support on 5709/5716");

static int force_netq = -1;
module_param(force_netq, int, 0);
MODULE_PARM_DESC(force_netq, "Enforce the number of NetQueues per port "
			     "(allowed values: 2-7, <2 is default value)");
...
static int bnx2_start_netqueue_hw(struct bnx2 *bp);
...
#if defined(__VMKLNX__) && defined(__VMKNETDDI_QUEUEOPS__)
	if ((bnapi->netq_flags & BNX2_NETQ_FREE_TX_QUEUE_STATE) &&
	    (sw_cons == txr->tx_prod)) {
		bp->netq_flags = L2_KCQE_OPCODE_VALUE_VM_FREE_TX_QUEUE;
...
#if defined(__VMKLNX__) && defined(__VMKNETDDI_QUEUEOPS__)
	int max_rings = (bp->dev->mtu <= 1500) ? RX_MAX_RINGS : 4;
	int msix_vecs = min(cpus + 1, max_rings);
	if(force_netq >= 2)
		msix_vecs = min(force_netq, RX_MAX_RINGS);
#else
...
bnx2_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (CHIP_NUM(bp) == CHIP_NUM_5709) {
		if (disable_netq == 0) {
			...
			printk(KERN_INFO "NetQueue Ops not registered\n");
	}
#endif
...
#if defined(VMWARE_ISCSI)
static vmk_IscsiNetDriverData iscsinet_bnx2_drv_data = {
	.drvData        = bnx2_cnic_probe,
	.privData       = NULL,
};
#endif
...
static int __init bnx2_init(void)
{
	int rc;
#if defined(VMWARE_ISCSI)
	VMK_ReturnStatus vm_err;
#endif

#if defined(__VMKLNX__) && defined(__VMKNETDDI_QUEUEOPS__)
	/*  sanity check the force_netq parameter */
	if(force_netq > 7) {
		printk(KERN_ERR "bnx2x: please use a 'force_netq'value "
				"between (2-7)\n");
		return -EINVAL;
	}
#endif
	...
	if (rc)
		return rc;
	vm_err = vmk_IscsiNetDriverRegister(THIS_MODULE->moduleID, &iscsinet_bnx2_drv_data);
	if (vm_err != VMK_OK) {
...
{
#if defined(VMWARE_ISCSI)
	vmk_IscsiNetDriverUnregister(THIS_MODULE->moduleID);
#endif
	pci_unregister_driver(&bnx2_pci_driver);
}
...
#define BNX2_NETQ_WAIT_EVENT_TIMEOUT	msecs_to_jiffies(500)

#define L2_KWQ_PAGE_CNT    4
#define L2_KCQ_PAGE_CNT    16
...
bnx2_service_netq_kcqes(struct bnx2_napi *bnapi, int num_cqes)
{
	...
	while (num_cqes) {
		...
		printk(KERN_INFO "kcwe service [reserved: 0x%x, status: 0x%x, qid: 0x%x, qe_self_seq: 0x%x, opcode: 0x%x, flags: 0x%x]\n",
...
bnx2_stop_netqueue_hw(struct bnx2 *bp)
{
	...
	/*  Return back all the used resources */
	bnx2_netqueue_free_resc(bp);
}
...
static int
bnx2_netq_alloc_rx_queue(struct net_device *netdev,
			 ...
			 struct napi_struct **napi_p)
{
	...
	for_each_nondefault_rx_queue(bp, i) {
		...
		if (!bnapi->rx_queue_allocated) {
			...
			rc = bnx2_netq_free_rx_queue_update(netdev, VMKNETDDI_QUEUEOPS_MK_RX_QUEUEID(i), 1);
	...
	printk(KERN_ERR "%s: No free rx queues found!\n", bp->dev->name);
	return VMKNETDDI_QUEUEOPS_ERR;
}

static int
bnx2_netq_alloc_tx_queue(struct net_device *netdev,
			 vmknetddi_queueops_queueid_t *p_qid,
			 u16 *queue_mapping)
{
	int i;
...
bnx2_netq_alloc_queue(vmknetddi_queueop_alloc_queue_args_t *args)
{
	...
	if (args->type == VMKNETDDI_QUEUEOPS_QUEUE_TYPE_TX) {
		return bnx2_netq_alloc_tx_queue(args->netdev, &args->queueid,
					     &args->queue_mapping);
	} else if (args->type == VMKNETDDI_QUEUEOPS_QUEUE_TYPE_RX) {
		return bnx2_netq_alloc_rx_queue(args->netdev, &args->queueid,
						&args->napi);
	} else {
		     ...
		     vmknetddi_queueops_queueid_t qid)
{
	...
	txr = &bnapi->tx_ring;
	if (txr->tx_cons == txr->tx_prod)
		goto  free_tx_ring;

	/*  Wait for the TX to flush all packets */
	bnapi->netq_flags |= BNX2_NETQ_FREE_TX_QUEUE_STATE;
	rc = wait_event_timeout(bp->netq_wait,
				(bp->netq_flags &
				 L2_KCQE_OPCODE_VALUE_VM_FREE_TX_QUEUE),
				BNX2_NETQ_WAIT_EVENT_TIMEOUT);
	bnapi->netq_flags &= ~BNX2_NETQ_FREE_TX_QUEUE_STATE;

	if (rc == 0) {
		printk(KERN_INFO  "%s: Timeout submiting free NetQ TX Queue: %x\n",
		       bp->dev->name, index);
...
bnx2_netq_free_rx_queue(struct net_device *netdev,
			vmknetddi_queueops_queueid_t qid)
{
	return bnx2_netq_free_rx_queue_update(netdev, qid, 0);
}
...
static int
bnx2_netq_remove_rx_filter(vmknetddi_queueop_remove_rx_filter_args_t *args)
{
	struct bnx2 *bp = netdev_priv(args->netdev);
	...
	if (fid != 0 || bnapi->rx_queue_active == 0)
		return VMKNETDDI_QUEUEOPS_ERR;
...
static int
bnx2_netq_apply_rx_filter(vmknetddi_queueop_apply_rx_filter_args_t *args)
{
	u8 *macaddr;
	struct bnx2_napi *bnapi;
	struct bnx2 *bp = netdev_priv(args->netdev);
	...
	if (class == VMKNETDDI_QUEUEOPS_FILTER_MACADDR) {
		...
		kwqe_set_rx_filter.vlan		= vlan_id;
	}
	...
	if (rc != 0) {
		printk(KERN_INFO "%s: NetQ set RX Filter: %d [%s %d]\n",
		       bp->dev->name,
		       queueid, print_mac(mac, macaddr), vlan_id);

		return VMKNETDDI_QUEUEOPS_OK;
	} else {
		printk(KERN_INFO "%s: Timeout submitting NetQ set RX Filter: %d [%s]\n",
		       bp->dev->name, queueid, print_mac(mac, macaddr));
