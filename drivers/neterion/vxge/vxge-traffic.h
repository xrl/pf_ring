...
#define VXGE_HW_MSG_SEND_RETRY          100
#define VXGE_HW_MSG_SEND_TO_VPATH_MASK  0xFFFFFFFFUL
#define VXGE_HW_CLEAR_INTR_SOURCE_ALL   0xFFFFFFFFFFFFFFFFULL
#define VXGE_HW_MSG_ENABLE_ALL          0xFFFFFFFFFFFFFFFFULL
#define VXGE_HW_MSG_TYPE_SEND_SVID_TO_VF        0x1
#define VXGE_HW_MSG_TYPE_SEND_CONFIG_GSO_TO_VF	0x3

#define VXGE_HW_MAX_VIRTUAL_FUNCTIONS	17
...
struct vxge_hw_xmac_stats {
	...
	struct vxge_hw_xmac_port_stats
				port_stats[VXGE_HW_MAC_MAX_MAC_PORT_ID];
	struct vxge_hw_xmac_vpath_tx_stats
...
/**
 * struct vxge_hw_vp_xmac_stats - XMAC Statistics
 *
 * @vpath_tx_stats: Per vpath XMAC TX stats
 * @vpath_rx_stats: Per vpath XMAC RX stats
 *
 * XMAC Statistics per vpath, to be used only by the
 * privilege adapter
 */
struct vxge_hw_vp_xmac_stats {
...
struct vxge_hw_device_stats_hw_info {
	struct vxge_hw_device_stats_mrpcim_info *mrpcim_info;
	struct vxge_hw_device_stats_mrpcim_info mrpcim_info_sav;
	struct vxge_hw_vpath_stats_hw_info
...
struct vxge_hw_vpath_stats_sw_ring_info {
	...
	/* LRO statistics */
	/* total no of Aggregated packets */
	...
	/* Number of times we reached upper packet limit
	 * for aggregation per session */
	...
	/* Max number of aggr packet  per ring */
	u64 lro_max_pkts_aggr;
	/* Average Aggregate packet */
...
struct vxge_hw_ring_rxd_info {
	...
	u32	proto;
	u32	is_vlan;
#define VXGE_HW_VLAN_VID_MASK 0xfff
	u32	vlan;
...
vxge_hw_vpath_doorbell_rx(
	struct __vxge_hw_ring *ringh, void *dtr_ptr);
...
enum vxge_hw_status vxge_hw_vpath_all_vid_enable(
	struct __vxge_hw_vpath_handle *vpath_handle);
...
#if !defined(VXGE_KERNEL_GRO)

#define	CONFIG_LRO_PUSH_TIMEOUT		100 /* micro seconds. */
#define	CONFIG_LRO_MAX_SG_NUM		5 /* For time being lets just take 2 */
#define	CONFIG_LRO_MAX_ACCUM_LENGTH	(64 * 1024)
#define	OS_NETSTACK_BUF			struct sk_buff*
#define vxge_os_in_multicast(addr)	IN_MULTICAST(addr)
#define VXGE_OS_INADDR_BROADCAST	INADDR_BROADCAST

/**
 * enum enum vxge_hw_frame_type - Ethernet frame format.
 * @VXGE_HW_FRAME_TYPE_DIX: DIX (Ethernet II) format.
 * @VXGE_HW_FRAME_TYPE_LLC: LLC format.
 * @VXGE_HW_FRAME_TYPE_SNAP: SNAP format.
 * @VXGE_HW_FRAME_TYPE_IPX: IPX format.
 *
 * Ethernet frame format.
 */
enum vxge_hw_frame_type {
	VXGE_HW_FRAME_TYPE_DIX			= 0x0,
	VXGE_HW_FRAME_TYPE_LLC			= 0x1,
	VXGE_HW_FRAME_TYPE_SNAP			= 0x2,
	VXGE_HW_FRAME_TYPE_IPX			= 0x3,
};
enum vxge_hw_tcp_option {

	VXGE_HW_TCPOPT_NOP = 1, /* Padding */
	VXGE_HW_TCPOPT_EOL = 0, /* End of options */
	VXGE_HW_TCPOPT_MSS = 2, /* Segment size negotiating */
	VXGE_HW_TCPOPT_WINDOW = 3, /* Window scaling */
	VXGE_HW_TCPOPT_SACK_PERM = 4, /* SACK Permitted */
	VXGE_HW_TCPOPT_SACK = 5, /* SACK Block */
	VXGE_HW_TCPOPT_TIMESTAMP = 8, /* Better RTT estimations/PAWS */
	VXGE_HW_TCPOPT_MD5SIG = 19, /* MD5 Signature (RFC2385) */
	VXGE_HW_TCPOLEN_TIMESTAMP = 10,
	VXGE_HW_TCPOLEN_TSTAMP_ALIGNED = 12

};

/**
 * struct vxge_hw_sw_lro - Software LRO Structure
 *
...
#define VXGE_HW_TS_SAVE 2
#define VXGE_HW_TS_VERIFY 1
#define VXGE_HW_TS_UPDATE 0

#define VXGE_HW_INET_ECN_MASK		3
#define	VXGE_HW_INET_ECN_CE		3
...
enum vxge_hw_status
__vxge_hw_sw_lro_capable(struct __vxge_hw_ring *ring, u8 *buffer,
	struct iphdr **ip,
	struct tcphdr **tcp,
	struct vxge_hw_ring_rxd_info *ext_info);

void vxge_hw_vpath_set_lro_sg_size(
	struct __vxge_hw_vpath_handle *vpath_handle, int lro_sg_size);
